...
public class ClientConnectionCloseEventProcessor implements ConnectionEventProcessor {
    private static final Logger LOGGER = LoggerFactory
                                           .getLogger(ClientConnectionCloseEventProcessor.class);
    ...
    public void onEvent(String remoteAddr, Connection conn) {
        if (null != conn) {
            LOGGER.info("[connection] Client disconnected, remote address: {}, localAddress: {}",
                remoteAddr, conn.getLocalAddress());
        }
    }
}
