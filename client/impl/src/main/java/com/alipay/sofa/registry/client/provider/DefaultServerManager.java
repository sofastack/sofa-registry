...
import com.alipay.sofa.registry.client.consistenthash.ConsistentHashLoadBalancer;
import com.alipay.sofa.registry.client.consistenthash.Invocation;
import com.alipay.sofa.registry.client.consistenthash.LoadBalancer;
...
public class DefaultServerManager implements ServerManager {
    ...
    public ServerNode consistentHash(String key) {
        ...
        LoadBalancer consistentHash = new ConsistentHashLoadBalancer();
        return consistentHash.select(urls, new Invocation(key));
    }
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    //先sleep后执行同步方法，是因为启动的时候会主动调用一次，避免冷启动。
                    syncServerList();
                } catch (Throwable e) {
