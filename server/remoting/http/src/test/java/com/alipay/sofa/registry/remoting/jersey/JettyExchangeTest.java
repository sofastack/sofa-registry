...
public class JettyExchangeTest {
  ...
  public void doTest() {
    ...
    JettyServer jettyServer =
        (JettyServer)
            jettyExchange.open(url, new Handler[] {JettyServer.createHandler(resourceConfig)});
  ...
  private void testJerseyJettyServer(
    ...
    Assert.assertEquals(new InetSocketAddress(JERSEY_TEST_PORT), jettyServer.getLocalAddress());
    Assert.assertFalse(jettyServer.isClosed());

    jettyServer.sendCallback(new JerseyChannel(null, null), new Object(), callbackHandler, 1000);
    Assert.assertNull(jettyServer.sendSync(new JerseyChannel(null, null), new Object(), 1000));
  }
