...
public class CRC16SlotFunction implements SlotFunction {
    ...
    public int slotOf(Object o) {
        return getCrc16Code(o) & slotMask;
    }

    private int getCrc16Code(Object o) {
        byte[] bytes = o.toString().getBytes(UTF8);
        int counter;
        int crc = 0;
        for (counter = 0; counter < bytes.length; counter++) {
            crc = (crc << 8) ^ crc16Tab[((crc >> 8) ^ bytes[counter]) & 0x00FF];
        }
        return crc;
    }
    ...
    private static final int[]   crc16Tab = { 0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50a5,
