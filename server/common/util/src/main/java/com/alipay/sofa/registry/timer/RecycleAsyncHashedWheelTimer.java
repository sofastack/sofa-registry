...
public class RecycleAsyncHashedWheelTimer extends AsyncHashedWheelTimer {
  ...
  public RecycleAsyncHashedWheelTimer(
  ...
  public Timeout newTimeout(
      TimerTask task, long recycleDelay, TimeUnit unit, BooleanSupplier checkCondition) {

    return super.newTimeout(
        new RecycleAsyncTimerTask(task, recycleDelay, unit, checkCondition), recycleDelay, unit);
  }
  ...
  class RecycleAsyncTimerTask implements TimerTask, Runnable {
    ...
    /** @param timerTask */
    public RecycleAsyncTimerTask(
        TimerTask timerTask, long recycleDelay, TimeUnit unit, BooleanSupplier checkCondition) {
      super();
    ...
    public void run() {
      try {
        ...
        if (checkCondition.getAsBoolean()) {
          RecycleAsyncHashedWheelTimer.this.newTimeout(
              timerTask, recycleDelay, delayUnit, checkCondition);
        }
      }
    }
  }
