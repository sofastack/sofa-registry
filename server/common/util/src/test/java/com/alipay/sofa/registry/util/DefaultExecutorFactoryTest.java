...
public class DefaultExecutorFactoryTest extends AbstractTestBase {
  ...
  public void testCreateCachedThreadPoolFactory() throws InterruptedException, TimeoutException {
    ThreadPoolExecutor executor = (ThreadPoolExecutor) DefaultExecutorFactory
            .createCachedThreadPoolFactory("test", 1, 10, TimeUnit.MILLISECONDS)
            .create();
    Assert.assertTrue(executor.getThreadFactory() instanceof NamedThreadFactory);
    Assert.assertTrue(((NamedThreadFactory)executor.getThreadFactory()).getNamePrefix().startsWith("test"));
    Assert.assertTrue(executor.getQueue() instanceof SynchronousQueue);
    int tasks = 10;
    new ConcurrentExecutor(tasks, executor).execute(new Runnable() {
      @Override
      public void run() {
        try {
          Thread.sleep(10);
  ...
  public void testBuilderWithQueueFactory() {
    DefaultExecutorFactory factory = DefaultExecutorFactory
            .builder()
            .workQueueFactory(new ObjectFactory<BlockingQueue<Runnable>>() {
              @Override
