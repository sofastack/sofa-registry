...
public class ScheduledSlotArranger extends AbstractLifecycleObservable implements
    ...
    public boolean arrangeSync() {
        if (metaLeaderService.amIStableAsLeader()) {
            ...
            logger.info("[arrangeSync] not stable leader for arrange, leader: [{}], is-leader: [{}], isWarmup [{}]",
                    metaLeaderService.getLeader(), metaLeaderService.amILeader(), metaLeaderService.isWarmup());
            return false;
        }
    }
}
