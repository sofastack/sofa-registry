    ...
    public void filter(ContainerRequestContext context) throws IOException {
        if (!metaLeaderService.amILeader()) {
            ...
            if (StringUtils.isEmpty(leaderIp)) {
                ...
                try {
                    LOGGER.warn("[filter]request: [{}][{}] need to redirect to {}",
                            getRemortIP(context), context.getMethod(), leaderIp);
                    response = Response.temporaryRedirect(
                            new URI("http://" + leaderIp + ":" + metaServerConfig.getHttpServerPort() + "/" + context.getUriInfo().getPath()))
                            .build();
                } catch (Throwable e) {
