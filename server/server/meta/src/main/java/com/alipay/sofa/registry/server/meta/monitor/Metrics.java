...
public class Metrics {
	...
	public static final class SlotArrange {
		...
		private static final Histogram SLOT_ARRANGE_HISTOGRAM = Histogram
				.build().namespace("meta").subsystem("slot_arrange")
				.name("leader_migrating_secs").help("migrating in seconds.")
				.buckets(3, 5, 10, 20, 30, 60, 120, 180, 240).register();
		...
		public static void end() {
			try {
				SLOT_ARRANGE_HISTOGRAM.observe(System.currentTimeMillis()
						- start);
	...
	public static final class Heartbeat {
		...
		private static final Gauge DATA_HEART_BEAT_GUAGE = Gauge.build()
				.namespace("meta").subsystem("heartbeat")
				.name("data_heartbeat").help("data heartbeat times")
				.labelNames("data_server").register();

		private static final Gauge SESSION_HEART_BEAT_GUAGE = Gauge.build()
				.namespace("meta").subsystem("heartbeat")
				.name("session_heartbeat").help("session heartbeat times")
				.labelNames("session_server").register();
	...
	public static final class SlotBalance {
		...
		private static final Gauge HIGH_LEADER_MIGRATE = Gauge.build()
				.namespace("meta").subsystem("slot_balancer")
				.name("high_leader_migrate").help("high leader migrate gauge")
				.labelNames("from", "to", "slot").register();

		private static final Gauge HIGH_LEADER_UPGRADE = Gauge.build()
				.namespace("meta").subsystem("slot_balancer")
				.name("high_leader_upgrade").help("high leader upgrade gauge")
				.labelNames("from", "to", "slot").register();

		private static final Gauge HIGH_FOLLOWER_BALANCE = Gauge.build()
				.namespace("meta").subsystem("slot_balancer")
				.name("high_follower_balance")
				.help("data to swap slot follower out")
				.labelNames("from", "to", "slot").register();

		private static final Gauge LOW_FOLLOWER_BALANCE = Gauge.build()
				.namespace("meta").subsystem("slot_balancer")
				.name("low_follower_balance")
				.help("data to swap slot follower in")
				.labelNames("from", "to", "slot").register();

		private static final Gauge LOW_LEADER_BALANCE = Gauge.build()
				.namespace("meta").subsystem("slot_balancer")
		...
		public static void onLeaderUpgrade(String from, String to, int slotId) {
			try {
				HIGH_LEADER_MIGRATE.labels(from, to, String.valueOf(slotId))
						.inc();
			} catch (Throwable throwable) {
		...
		public static void onLeaderMigrate(String from, String to, int slotId) {
			try {
				HIGH_LEADER_UPGRADE.labels(from, to, String.valueOf(slotId))
						.inc();
			} catch (Throwable throwable) {
		...
		public static void onHighFollowerMigrate(String from, String to,
				int slotId) {
			try {
				HIGH_FOLLOWER_BALANCE.labels(from, to, String.valueOf(slotId))
						.inc();
			} catch (Throwable throwable) {
		...
		public static void onLowFollowerMigrate(String from, String to,
				int slotId) {
			try {
				LOW_FOLLOWER_BALANCE.labels(from, to, String.valueOf(slotId))
						.inc();
			} catch (Throwable throwable) {
		...
		public static void onLowLeaderReplace(String from, String to, int slotId) {
			try {
				LOW_LEADER_BALANCE.labels(from, to, String.valueOf(slotId))
						.inc();
			} catch (Throwable throwable) {
	...
	public static final class SlotAssign {
		...
		private static final Gauge LEADER_ASSIGN_GAUGE = Gauge.build()
				.namespace("meta").subsystem("slot_assigner")
				.name("assign_leader").help("leader assign")
				.labelNames("leader", "slot").register();

		private static final Gauge FOLLOWER_ASSIGN_GAUGE = Gauge.build()
				.namespace("meta").subsystem("slot_assigner")
				.name("assign_follower").help("follower assign")
				.labelNames("follower", "slot").register();

		public static void onSlotLeaderAssign(String dataServer, int slotId) {
			try {
				LEADER_ASSIGN_GAUGE.labels(dataServer, String.valueOf(slotId))
						.inc();
			} catch (Throwable throwable) {
		...
		public static void onSlotFollowerAssign(String dataServer, int slotId) {
			try {
				FOLLOWER_ASSIGN_GAUGE
						.labels(dataServer, String.valueOf(slotId)).inc();
			} catch (Throwable throwable) {
	...
	public static final class DataSlot {
		...
		private static final Gauge LEADER_ASSIGN_GAUGE = Gauge.build()
				.namespace("meta").subsystem("data").name("leader_slot_number")
				.help("leader slot number").labelNames("data_server")
				.register();

		private static final Gauge FOLLOWER_ASSIGN_GAUGE = Gauge.build()
				.namespace("meta").subsystem("data")
				.name("follower_slot_number").help("follower slot number")
				.labelNames("data_server").register();

		private static final Gauge DATA_SLOT_TABLE_LAG_TIMES = Gauge.build()
				.namespace("meta").subsystem("data")
				.name("data_server_slot_lag").help("data server slot is behind meta")
				.labelNames("data_server").register();

		private static final Gauge DATA_SLOT_GREATER_THAN_META = Gauge.build()
				.namespace("meta").subsystem("data")
				.name("data_slot_greater_than_meta").help("data server slot epoch greater than meta")
				.labelNames("data_server", "slotId").register();

		private static final Gauge DATA_SLOT_NOT_EQUALS_META = Gauge.build()
				.namespace("meta").subsystem("data")
				.name("data_slot_not_equals_meta").help("data server slot is not as expected as meta")
				.labelNames("data_server").register();
