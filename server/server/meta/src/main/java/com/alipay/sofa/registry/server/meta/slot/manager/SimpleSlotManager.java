...
public class SimpleSlotManager extends AbstractLifecycleObservable implements SlotManager {
    ...
    private final AtomicReference<SlotTableCacheWrapper> localRepo = new AtomicReference<>(new SlotTableCacheWrapper(SlotTable.INIT, ImmutableMap.of()));
            ...
            if (slotTable.getEpoch() <= localRepo.get().slotTable.getEpoch()) {
                if (logger.isWarnEnabled()) {
                    logger.warn(
                        ...
                        slotTable.getEpoch(), localRepo.get().slotTable.getEpoch());
                }
                return;
            }
