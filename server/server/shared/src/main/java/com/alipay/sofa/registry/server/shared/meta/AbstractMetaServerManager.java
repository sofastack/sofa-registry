 ...
 * Mar 15, 2021
 */
public abstract class AbstractMetaServerManager extends ClientSideExchanger implements MetaServerManager, Initializable, Startable {
    ...
    protected void doRefreshConnections() {
        ...
        diff.accept(new ComparatorVisitor<String>() {
            @Override
    ...
    public Response request(Request request) throws RequestException {
        LOGGER.info("[request] MetaNode Exchanger request={},url={},callbackHandler={}",
        ...
        try {
            ...
            LOGGER.warn(
                    "[request] MetaNode Exchanger request send error!It will be retry once!Request url:{}", url);
            return super.request(request);
        }
    }
    ...
    private List<String> getConfiguredMetaIp() {
        ...
        metaDomains.forEach(domain -> {
            ...
            if (ip == null) {
                throw new SofaRegistryRuntimeException("Node config convert domain {" + domain + "} error!");
            }
            set.add(ip);
        });
