...
@Fork(1)
public class ChangeWorkerGetExpireBenchmark {
  ...
  private Thread commitChangeTaskThread;

  public ChangeWorkerGetExpireBenchmark() {
  ...
  public void tearDown() {
    this.commitChangeTaskThread.interrupt();
    this.commitChangeTaskThread = null;
  }

  private void commitChangeTask() {
    ...
    if (dataInfoNumber >= this.largeDataInfoIdThreshold) {
      changeCtx = new TriggerPushContext(DATA_CENTER, version, DATA_NODE, 0, traceTimes, 1200);
    } else {
  ...
  @Benchmark
  public void benchmarkGetExpireTask() {
    this.changeWorker.getExpireTasks();
  }

  class CommitChangeTaskRunnable implements Runnable {
    @Override
    public void run() {
      while (true) {
        ChangeWorkerGetExpireBenchmark.this.commitChangeTask();
