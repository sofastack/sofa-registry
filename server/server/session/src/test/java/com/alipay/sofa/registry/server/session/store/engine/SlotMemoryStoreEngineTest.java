...
/** 测试支持按Key进行分槽的存储引擎 */
public class SlotMemoryStoreEngineTest {

  @Test
  public void test() {
    SlotMemoryStoreEngine<Watcher> storeEngine = new SlotMemoryStoreEngine<>(String::hashCode);
    ...
    int slotId = (dataInfoId + "00").hashCode();
    Collection<Watcher> slotWatchers = storeEngine.getSlotStoreData(slotId);
    Assert.assertEquals(2, slotWatchers.size());
  }
}
