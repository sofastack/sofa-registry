 ...
 * 抽象变更任务处理 Worker
 * <p>
 * 作为所有变更任务处理器的基类，提供了通用的任务管理功能：
 * 1. 线程安全保障机制
 * 2. 防抖延迟控制
 * 3. 任务提交、查找和过期任务获取的基础框架
 * <p>
 * 子类需要实现具体的任务存储和检索逻辑
 */
public abstract class AbstractChangeWorker implements ChangeWorker<ChangeKey, ChangeTaskImpl> {
   ...
   */
  public AbstractChangeWorker(int changeDebouncingMillis, int changeDebouncingMaxMillis, int changeTaskWaitingMillis) {
   ...
   * @param task 要添加的任务对象
   */
  protected abstract void pushTask(ChangeKey key, ChangeTaskImpl task);
   ...
   * @param task 要更新的任务对象
   */
  protected abstract void updateTask(ChangeKey key, ChangeTaskImpl task);

  /**
   * 默认的任务合并逻辑实现
   * 子类可以根据需要重写此方法以实现特定的合并策略
   *
   * @param key       任务键
   * @param task      新任务对象
   * @param handler   任务处理器
  ...
  protected ChangeTaskImpl createTask(ChangeKey key, TriggerPushContext changeCtx, ChangeHandler handler, long now) {
    ChangeTaskImpl changeTaskImpl = new ChangeTaskImpl(key, changeCtx, handler, now + this.changeDebouncingMillis);
