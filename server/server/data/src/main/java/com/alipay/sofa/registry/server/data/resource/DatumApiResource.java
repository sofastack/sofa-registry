...
public class DatumApiResource {
     ...
     *  curl -i -d '{"dataInfoId":"testDataId#@#DEFAULT_INSTANCE_ID#@#DEFAULT_GROUP", "publisherDataBox":"1234", "publisherCell":"TestZone2"} ' -H "Content-Type: application/json" -X POST http://localhost:9622/datum/api/pub/add
     *  "publisherConnectId" can be unspecified. If not specified, the connectId of the first pub under the datum can be automatically selected (for testing purposes, it is generally not necessary to specify the connectId artificially).
     */
    @POST
    @Path("/pub/add")
    @Produces(MediaType.APPLICATION_JSON)
    ...
    public Object getDatumVersion(DatumParam datumParam) {
        ...
        return String.format(format, datumParam.getDataCenter(), datumParam.getDataInfoId(),
            dataServer, version);
    }

    private CommonResponse getNotFoundResponse(DatumParam datumParam) {
        return CommonResponse.buildFailedResponse(String.format(
            "datum not found by dataCenter=%s, dataInfoId=%s", datumParam.getDataCenter(),
    ...
    private Publisher buildPublisher(Datum datum, DatumParam datumParam) {
        Validate.notNull(datumParam.getPublisherRegisterId(),
            "datumParam.publisherRegisterId is null");
        ...
        if (datumParam.getPublisherConnectId() != null) {
            String[] parts = datumParam.getPublisherConnectId().split(
    ...
    private UnPublisher buildUnPublisher(Datum datum, DatumParam datumParam) {
        Validate.notNull(datumParam.getPublisherRegisterId(),
            "datumParam.publisherRegisterId is null");
        Validate.notNull(datumParam.getPublisherRegisterTimestamp(),
            "datumParam.publisherRegisterTimestamp is null");

        UnPublisher publisher = new UnPublisher(datumParam.getDataInfoId(), TEMP_SESSION_PROCESSID,
            datumParam.getPublisherRegisterId(), datumParam.getPublisherRegisterTimestamp(),
            datumParam.getPublisherVersion());

        Publisher _publisher = datum.getPubMap().get(datumParam.getPublisherRegisterId());
        if (_publisher == null) {
            throw new IllegalArgumentException(String.format("No pub of registerId(%s)",
    ...
    private CommonResponse createResponse(Datum datum) {
        return CommonResponse.buildSuccessResponse(String.format("datum=%s, publishers=%s", datum,
            datum.getPubMap()));
    }
}