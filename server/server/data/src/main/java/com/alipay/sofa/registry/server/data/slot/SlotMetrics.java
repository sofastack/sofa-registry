...
public final class SlotMetrics {
	...
	static final class Manager {
		private static final Counter LEADER_UPDATE_COUNTER = Counter.build()
				.namespace("data").subsystem("slot")
				.name("leader_update_total").help("count leader update")
				.register();

		private static final Gauge LEADER_ASSIGN_GAUGE = Gauge.build()
				.namespace("data").subsystem("slot")
				.name("leader_assign_total").help("leader assign").register();

		private static final Gauge FOLLOWER_ASSIGN_GAUGE = Gauge.build()
				.namespace("data").subsystem("slot")
				.name("follower_assign_total").help("follower assign")
				.register();

		private static final Gauge LEADER_MIGRATING_GAUGE = Gauge.build()
				.namespace("data").subsystem("slot")
				.name("leader_migrating_total")
				.help("count leader is migrating").labelNames("slot")
				.register();

		private static final Counter LEADER_MIGRATING_FAIL_COUNTER = Counter
				.build().namespace("data").subsystem("slot")
				.name("leader_migrating_fail_total")
				.help("count leader migrating fail")
				.labelNames("slot", "session").register();
		...
		static void observeLeaderMigratingFail(int slotId, String sessionIp) {
			LEADER_MIGRATING_FAIL_COUNTER.labels(String.valueOf(slotId),
					sessionIp).inc();
		}

		static void observeLeaderMigratingHistogram(int slotId, long millis) {
			// seconds
			LEADER_MIGRATING_HISTOGRAM.labels(String.valueOf(slotId)).observe(
					millis / 1000d);
		}
	}

	static final class SyncSession {
		private static final Counter SYNC_SESSION_ID_COUNTER = Counter.build()
				.namespace("data").subsystem("sync").name("session_id_total")
				.help("count sync session dataInfoIds").labelNames("slot")
				.register();

		private static final Counter SYNC_SESSION_ID_NUM_COUNTER = Counter
				.build().namespace("data").subsystem("sync")
				.name("session_id_num_total")
				.help("count sync session dataInfoId's num").labelNames("slot")
				.register();

		private static final Counter SYNC_SESSION_PUB_COUNTER = Counter.build()
				.namespace("data").subsystem("sync").name("session_pub_total")
				.help("count sync session pubs").labelNames("slot").register();

		private static final Counter SYNC_SESSION_PUB_NUM_COUNTER = Counter
				.build().namespace("data").subsystem("sync")
				.name("session_pub_num_total")
				.help("count sync session pub's num").labelNames("slot")
				.register();
	...
	static final class SyncLeader {
		private static final Counter SYNC_LEADER_ID_COUNTER = Counter.build()
				.namespace("data").subsystem("sync").name("leader_id_total")
				.help("count sync leader dataInfoIds").labelNames("slot")
				.register();

		private static final Counter SYNC_LEADER_ID_NUM_COUNTER = Counter
				.build().namespace("data").subsystem("sync")
				.name("leader_id_num_total")
				.help("count sync leader dataInfoId's num").labelNames("slot")
				.register();

		private static final Counter SYNC_LEADER_PUB_COUNTER = Counter.build()
				.namespace("data").subsystem("sync").name("leader_pub_total")
				.help("count sync leader pubs").labelNames("slot").register();

		private static final Counter SYNC_LEADER_PUB_NUM_COUNTER = Counter
				.build().namespace("data").subsystem("sync")
				.name("leader_pub_num_total")
				.help("count sync leader pub's num").labelNames("slot")
				.register();
